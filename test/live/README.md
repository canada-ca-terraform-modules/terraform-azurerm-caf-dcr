...
running. It is **not** a substitute for either of the module's other two test
surfaces:
...
| File | Purpose |
|---|---|
| `main.tf` | Module block with `source = "../../"` (a relative path, not a pinned `?ref` - "baseline" and "PR" are just two on-disk checkouts of this repo), the `azurerm` provider config, an empty `backend "local" {}` block (path supplied at `init` time - see below), and a `local.dcr` override that splices the real Log Analytics workspace ID from `test_dependencies.tf` into `var.dcr.destinations.log_analytics` (not knowable statically in a tfvars fixture). |
| `test_dependencies.tf` | A dedicated, throwaway resource group + Log Analytics workspace this harness owns outright - never shared/production infra. Names are suffixed with `var.pr_number` so concurrently open PRs never collide. |
| `variables.tf` | `env`, `group`, `project`, `userDefinedString`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), `repository`, and `dcr` (typed `any`, passed straight through to the module). |
| `config/dcr.tfvars` | One representative real-usage fixture: a Linux-kind DCR with a Log Analytics destination, a syslog data source, and a matching data flow. |
...
cd test/live
terraform init
terraform plan -var-file=config/dcr.tfvars
terraform apply -var-file=config/dcr.tfvars
```

Confirm only the live-test resource group/workspace and `module.dcr` are
planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/dcr.tfvars
```
...
terraform apply -var-file=config/dcr.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/dcr.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/dcr.tfvars -var="pr_number=<pr-number>"
```
